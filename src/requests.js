...
export function getNotes(callback) {
  sendRequest(`${route}/api/v1/notes`).then(callback).catch(function (error){
    console.error(`Impossible to get notes: ${error}`)
  });
}

export function createNote(callback, body) {
  sendRequest(`${route}/api/v1/notes`,'POST', body).then(callback).catch(function(error){
    console.error(`Impossible to create note: ${error}`)
  });
}

export function deleteNote(callback, id) {
  sendRequest(`${route}/api/v1/notes/${id}`,'DELETE').then(callback).catch(function(error){
